...
import numpy as np
import zarr
from dask import delayed, compute  # 添加delayed导入
...
class CPAAnalysis(PPBasic):
    ...
    def perform_cpa(self):
        ...
        # print("Correlation type:", type(correlation))
        
        # traces = self.t[:self.sel_num_traces, self.sample_range[0]:self.sample_range[1]] # 原始版本
        traces = self.t[:self.sel_num_traces, self.sample_range[0]:self.sample_range[1]].compute()
        # print("Traces type:", type(traces))
        # print("Self sample range type:", type(self.sample_range))
        
        if self.dr == 'enc':
            plaintext_bytes = self.plaintext[:self.sel_num_traces, :16].compute()
            # print("Plaintext bytes type:", type(plaintext_bytes))
        elif self.dr == 'dec':
            ciphertext_bytes = self.ciphertext[:self.sel_num_traces, :16].compute()
            # print("Ciphertext bytes type:", type(ciphertext_bytes))
        ...
        def compute_correlation(key_byte):
            ...
            # print("Key byte array type:", type(key_byte_array))
            if self.dr == 'enc':            
            ...
            # print("sbox output type:", type(sbox_output))
            # print("HW matrix type:", type(hw_matrix))
            trace_count = self.sel_num_traces
            # print("trace count type:", type(trace_count))
            
            # 使用NumPy直接计算，确保数学正确性
            sample_mean = np.mean(traces, axis=0)
            model_mean = np.mean(hw_matrix, axis=0)
            # print("Sample mean type:", type(sample_mean))
            # print("Model mean type:", type(model_mean))
            
            # 向量化计算协方差矩阵 - 大幅优化性能
            # 计算 hw_matrix 和 traces 的乘积和
            prod_sum = np.dot(hw_matrix.T, traces)
            # print("Product sum type:", type(prod_sum))
            
            # 计算协方差矩阵
            cov_matrix = (prod_sum / trace_count) - np.outer(model_mean, sample_mean)
            # print("Covariance matrix type:", type(cov_matrix))
            
            # 计算标准差
            model_var = np.maximum(0, np.var(hw_matrix, axis=0))
            sample_var = np.maximum(0, np.var(traces, axis=0))
            # print("Model var type:", type(model_var))
            # print("Sample var type:", type(sample_var))
            
            model_std = np.sqrt(model_var)
            sample_std = np.sqrt(sample_var)
            # print("Model std type:", type(model_std))
            # print("Sample std type:", type(sample_std))
            ...
            # print("Correlation result type:", type(correlation_result))
            
            return correlation_result

        # 并行计算所有密钥字节
        delayed_results = [delayed(compute_correlation)(kb) for kb in range(256)]
        with ProgressBar():
            da_results = list(compute(*delayed_results))
            results = da_results
        ...
        for key_byte, result in enumerate(results):
            # print(len(result), result.shape, type(result))
            correlation[key_byte,:,self.sample_range[0]:self.sample_range[1]] = result # 原始版本
